...
class MLP(Layer):
    ...
    def __init__(
        ...
        for i, hidden_feature in enumerate(hidden_features[:-1]):  # w/o last one for IndexOutOfBounds exception
            layers.append(Linear(hidden_feature, hidden_features[i + 1]))
            layers.append(activation_cls())
